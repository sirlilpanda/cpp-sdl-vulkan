...
struct Vertex {
    ...
    static VkVertexInputBindingDescription getBindingDescription() {
        VkVertexInputBindingDescription bindingDescription{};
            ...
            // change this for instance rendering
...
static int resizingEventWatcher(void* data, SDL_Event* event) {
  ...
  if (event->type == SDL_WINDOWEVENT && event->window.event == SDL_WINDOWEVENT_RESIZED) {
    auto app = reinterpret_cast<HelloTriangleApplication*>(SDL_GetWindowFromID(event->window.windowID));
    app->framebufferResized = true;
    int height, width;
    SDL_GetWindowSize(SDL_GetWindowFromID(event->window.windowID), &width, &height);
    SDL_Log("resized to w:%d h:%d", width, height);
  }
...
std::vector<const char*> getRequiredExtensions() {
    ...
    SDL_Vulkan_GetInstanceExtensions(window, &extension_count, &extension_names[0]);
...
// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
//  put custom logging formatting here
// @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
/// @brief 
/// @param messageSeverity which is one of the following flags
...
/// @param pCallbackData refers to a VkDebugUtilsMessengerCallbackDataEXT struct 
/// containing the details of the message itself, with the most important members being
/// • pMessage: The debug message as a null-terminated string
/// • pObjects: Array of Vulkan object handles related to the message
/// • objectCount: Number of objects in array
/// @param pUserData parameter contains a pointer that was specified during the setup of the callback and allows you to pass your own data to it.
/// @return oolean that indicates if the Vulkan call that triggered
...
void createInstance() {
    ...
    VkApplicationInfo app_info{};
        ...
        app_info.pEngineName = "shrimp";
        app_info.engineVersion = VK_MAKE_VERSION(1, 0, 0);
        app_info.apiVersion = VK_API_VERSION_1_0;

    // getting all the extensions from sdl    
    auto sdl_extensions = getRequiredExtensions();

    // checking for avaible extensions
    uint32_t extension_count;
    ...
    SDL_Log("creating instance_info");
    ...
    SDL_Log("created instance_info");
    
    VkResult res = vkCreateInstance(&instance_info, nullptr, &instance);

    if (res != VK_SUCCESS){
        SDL_Log("failed to create vulkan instance got error %d", res);
...
void createSwapChain() {
    ...
    // UNCOMMENT FOR POST PROCESSING TINGS G
    ...
    if (res != VK_SUCCESS) {
        SDL_Log("vkCreateSwapchainKHR error: %d", res);
...
/// find out what the gpu support here
